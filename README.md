...
Inside a container, `localhost` refers to that same container only. Containers must communicate using service names.
...
When the frontend initiates a WebSocket connection to `/ws`, NGINX forwards it to the backend container:
...
2. Connects to EC2 server using SSH
...
The backend expected frontend files under `/frontend/index.html`, but the frontend directory was not copied into the image.
...
This project was deployed on **AWS EC2**.
